<template><div><h1 id="javascript、nodejs、typescript" tabindex="-1"><a class="header-anchor" href="#javascript、nodejs、typescript"><span>JavaScript、NodeJs、TypeScript</span></a></h1>
<h2 id="javascript" tabindex="-1"><a class="header-anchor" href="#javascript"><span>JavaScript</span></a></h2>
<h3 id="闭包-closure" tabindex="-1"><a class="header-anchor" href="#闭包-closure"><span>闭包(closure)</span></a></h3>
...
<span class="line"></span></code></pre>
</div><h3 id="事件循环机制" tabindex="-1"><a class="header-anchor" href="#事件循环机制"><span>事件循环机制</span></a></h3>
...
<h3 id="栈-stack-和堆-heap" tabindex="-1"><a class="header-anchor" href="#栈-stack-和堆-heap"><span>栈(Stack)和堆(Heap)</span></a></h3>
<p>栈存放着基本数据类型，因为这些数据大小固定，可以直接在内存中分配空间。
堆存放着复杂数据类型如对象、数组、函数等，因为这些大小可能是动态的。声明一个对象时，会将对象的数据放在堆中，栈中会存放着对象数据的内存引用(指针)</p>
<h3 id="模块化规范" tabindex="-1"><a class="header-anchor" href="#模块化规范"><span>模块化规范</span></a></h3>
<ul>
<li>CommonJS：Node.js采用的模块化规范，同步加载模块，通过module.exports导出模块，通过require引入模块</li>
<li>AMD：异步模块定义，用于浏览器环境，通过requirejs实现</li>
<li>ES6 Module：ES6引入的模块化规范，通过export导出模块，通过import引入模块，编译时调用必须放在文件开头</li>
</ul>
<table>
<thead>
<tr>
<th></th>
<th>CommonJs(基于文件读写)</th>
<th>EsModule(基于请求)</th>
</tr>
</thead>
<tbody>
<tr>
<td>导入方式</td>
<td>require()动态加载模块，同步执行，在node环境中，它会立即读取并执行模块文件</td>
<td>静态导入使用import语法动态导入使用import函数，静态导入必须位于顶层作用域，动态可以按需加载</td>
</tr>
<tr>
<td>构建模块依赖关系</td>
<td>require同步加载并执行模块文件，在实际运行中构建</td>
<td>编译阶段建立模块关联关系，预处理阶段分析模块依赖，执行阶段执行模块代码</td>
</tr>
<tr>
<td>输出的值</td>
<td>导出值时(string、number)是值的拷贝，如果是一个对象或者函数，则是对象或者函数的引用，</td>
<td>输出是实时的只读引用，内部变化会影响外部，export和import创建了一种双向绑定机制</td>
</tr>
</tbody>
</table>
<h4 id="umd规范-universal-module-definition" tabindex="-1"><a class="header-anchor" href="#umd规范-universal-module-definition"><span>UMD规范（Universal Module Definition）</span></a></h4>
<p><strong>JS通用模块规范UMD</strong>主流的javascript模块规范：CommonJS，UMD(AMD，CMD:已经废弃，因为需要声明引入)，模块最终都是要导出一个对象，函数，或者变量，不同的规范导出这部分的定义不同</p>
<div class="language-javascript" data-highlighter="prismjs" data-ext="js" data-title="js"><pre v-pre><code><span class="line"><span class="token punctuation">(</span><span class="token keyword">function</span><span class="token punctuation">(</span><span class="token parameter">root<span class="token punctuation">,</span> factory</span><span class="token punctuation">)</span> <span class="token punctuation">{</span></span>
<span class="line">    <span class="token keyword">if</span><span class="token punctuation">(</span><span class="token keyword">typeof</span> modele <span class="token operator">===</span> <span class="token string">'object'</span> <span class="token operator">&amp;&amp;</span> <span class="token keyword">typeof</span> module<span class="token punctuation">.</span>exports <span class="token operator">===</span> <span class="token string">'object'</span><span class="token punctuation">)</span> <span class="token punctuation">{</span></span>
<span class="line">        <span class="token comment">// commonjs规范， nodejs环境</span></span>
<span class="line">        <span class="token keyword">var</span> depModele <span class="token operator">=</span> <span class="token function">require</span><span class="token punctuation">(</span><span class="token string">''</span><span class="token punctuation">)</span></span>
<span class="line">        module<span class="token punctuation">.</span>exports <span class="token operator">=</span> <span class="token function">factory</span><span class="token punctuation">(</span>depModule<span class="token punctuation">)</span></span>
<span class="line">    <span class="token punctuation">}</span><span class="token keyword">else</span> <span class="token keyword">if</span> <span class="token punctuation">(</span><span class="token keyword">typeof</span> define <span class="token operator">===</span> <span class="token string">'function'</span> <span class="token operator">&amp;&amp;</span> define<span class="token punctuation">.</span>amd<span class="token punctuation">)</span> <span class="token punctuation">{</span></span>
<span class="line">        <span class="token comment">// AMD规范，如require.js</span></span>
<span class="line">        <span class="token function">define</span><span class="token punctuation">(</span><span class="token punctuation">[</span><span class="token string">'depModule'</span><span class="token punctuation">]</span><span class="token punctuation">,</span> factory<span class="token punctuation">)</span></span>
<span class="line">    <span class="token punctuation">}</span><span class="token keyword">else</span> <span class="token keyword">if</span> <span class="token punctuation">(</span><span class="token keyword">typeof</span> define <span class="token operator">===</span> <span class="token string">'function'</span> <span class="token operator">&amp;&amp;</span> define<span class="token punctuation">.</span>cmd<span class="token punctuation">)</span> <span class="token punctuation">{</span></span>
<span class="line">        <span class="token comment">//CMD模块规范， 如sea.js</span></span>
...
<span class="line">            <span class="token keyword">var</span> depModule <span class="token operator">=</span> <span class="token function">require</span><span class="token punctuation">(</span><span class="token string">'depModule'</span><span class="token punctuation">)</span></span>
<span class="line">            module<span class="token punctuation">.</span>exports <span class="token operator">=</span> <span class="token function">factory</span><span class="token punctuation">(</span>depModule<span class="token punctuation">)</span></span>
<span class="line">        <span class="token punctuation">}</span><span class="token punctuation">)</span></span>
<span class="line">    <span class="token punctuation">}</span><span class="token keyword">else</span> <span class="token punctuation">{</span></span>
<span class="line">        <span class="token comment">// 没有规范 直接挂全局对象</span></span>
<span class="line">        root<span class="token punctuation">.</span>undModule <span class="token operator">=</span> <span class="token function">factory</span><span class="token punctuation">(</span>root<span class="token punctuation">,</span> deppModule<span class="token punctuation">)</span></span>
<span class="line">    <span class="token punctuation">}</span></span>
<span class="line"><span class="token punctuation">}</span><span class="token punctuation">)</span><span class="token punctuation">(</span><span class="token keyword">this</span><span class="token punctuation">,</span> <span class="token keyword">function</span><span class="token punctuation">(</span><span class="token parameter">depModeule</span><span class="token punctuation">)</span> <span class="token punctuation">{</span></span>
<span class="line">    <span class="token keyword">return</span> <span class="token punctuation">{</span></span>
<span class="line">            <span class="token literal-property property">standards</span><span class="token operator">:</span> <span class="token string">''</span></span>
<span class="line">    <span class="token punctuation">}</span></span>
<span class="line"><span class="token punctuation">}</span><span class="token punctuation">)</span></span>
<span class="line"></span></code></pre>
</div><h3 id="浏览器本地储存" tabindex="-1"><a class="header-anchor" href="#浏览器本地储存"><span>浏览器本地储存</span></a></h3>
<p><strong>cookie</strong>：浏览器和js-cookie可生成，可以设置过期时效，最大支持4K，每次请求携带在header里，字符串键值对在本地存储数据</p>
<p><strong>localStorage</strong>：除非清除，则一直存在，不支持跨浏览器，最大支持5M，不参与请求，字符串键值对存储本地</p>
<p><strong>sessionStorage</strong>：页面关闭清理，不支持跨页面，最大5M，不参与请求，字符串键值对存储本地</p>
<p><strong>indexedDB</strong>：除非清理，则一直存在，不限制大小，不参与请求，非关系型数据库（不支持sql语句操作），提供接口查询，建立索引</p>
<ul>
...
<h3 id="javascript遍历方法" tabindex="-1"><a class="header-anchor" href="#javascript遍历方法"><span>Javascript遍历方法</span></a></h3>
<h4 id="对象遍历" tabindex="-1"><a class="header-anchor" href="#对象遍历"><span>对象遍历</span></a></h4>
<ul>
<li>for-in: 遍历对象自身和继承的可以枚举属性 不含Symbol</li>
<li>Object.keys() Object.values() Object.entries()</li>
<li>for-of: for(let value of Object.key（obj）){}</li>
<li>Object.getOwnPropertyNames()</li>
<li>Object.getOwnPropertySymbols()</li>
<li>Reflect.ownKeys()</li>
</ul>
<h4 id="数组遍历" tabindex="-1"><a class="header-anchor" href="#数组遍历"><span>数组遍历</span></a></h4>
<ul>
<li>for: for (let i=0, i &lt; arr.length, i++) {}</li>
<li>while: while (i &lt; arr.length) {}</li>
<li>for-in: for (let i in arr) {}</li>
<li>forEach: arr.forEach( (val, index, arr)=&gt; {})，不支持处理异步函数，无法捕获函数中的错误，无法跳出中断执行，删除自身元素index不会被重置，函数签名中包含参数和上下文性能低</li>
<li>map: arr.map() 需要配合return 完成返回值</li>
<li>filter: arr.filter() return 返回操作满足条件的</li>
<li>some: arr.some return 元素有一个符合条件结果返回true或者false</li>
<li>every: arr.every() return 元素所以符合条件时返回true或者false</li>
<li>reduce: arr.reduce((total, value, index, arr)=&gt; {})</li>
<li>ruduceRight: arr.ruduceRight()</li>
<li>for-or: for(let val of arr){} 适合遍历数、数组对象、字符串、map、set、 等拥有迭代器对象的集合</li>
<li>find: arr.find() 找到第一个符合条件的元素返回 没有返回undefined</li>
<li>findIndex: arr.findIndex() 找到第一个符合元素的返回索引 没有返回-1</li>
<li>includes: arr.includes() 判断数组中是否包含某元素 返回ture false</li>
</ul>
<h3 id="特性和问题" tabindex="-1"><a class="header-anchor" href="#特性和问题"><span>特性和问题</span></a></h3>
<h4 id="异常" tabindex="-1"><a class="header-anchor" href="#异常"><span>异常</span></a></h4>
<ul>
...
<li>EvalError: Eval方法错误</li>
<li>URLError: url相关方法产生的错误</li>
</ul>
<div class="hint-container tip">
<p class="hint-container-title">提示</p>
<p>调试技巧，元素右键保存到全局变量，控制台可以看到这个变量并使用他的属性，比如输入框可以看到 input.value元素面板里时间监听可以找到对应的事件绑定在代码多少行</p>
</div>
<h4 id="请求参数类型" tabindex="-1"><a class="header-anchor" href="#请求参数类型"><span>请求参数类型</span></a></h4>
<ul>
<li>query 查询参数是附加在 URL 后面的一系列键值对（?key1=value1&amp;key2=value2）</li>
<li>params 路径参数是直接嵌入到 URL 路径中的变量部分 (/resource/{id}/{name})</li>
<li>body 请求体是在 HTTP 请求的消息体中发送的数据 (1.urlencode 2.json )</li>
</ul>
<h4 id="精度丢失" tabindex="-1"><a class="header-anchor" href="#精度丢失"><span>精度丢失</span></a></h4>
<p>当Number类型大于2^53-1就会出现精度丢失</p>
<div class="language-javascript" data-highlighter="prismjs" data-ext="js" data-title="js"><pre v-pre><code><span class="line"><span class="token comment">// 正则替换</span></span>
...
<span class="line">    <span class="token keyword">const</span> convertedJsonString <span class="token operator">=</span> data<span class="token punctuation">.</span><span class="token function">replace</span><span class="token punctuation">(</span><span class="token regex"><span class="token regex-delimiter">/</span><span class="token regex-source language-regex">"(\w+)":(\d{15,})</span><span class="token regex-delimiter">/</span><span class="token regex-flags">g</span></span><span class="token punctuation">,</span><span class="token string">'"$1":"$2"'</span><span class="token punctuation">)</span></span>
<span class="line">    <span class="token keyword">return</span> <span class="token constant">JSON</span><span class="token punctuation">.</span><span class="token function">parse</span><span class="token punctuation">(</span>convertedJsonString<span class="token punctuation">)</span></span>
<span class="line">  <span class="token punctuation">}</span><span class="token punctuation">]</span></span>
<span class="line"><span class="token punctuation">}</span><span class="token punctuation">)</span></span>
...
<span class="line">  url<span class="token punctuation">,</span></span>
<span class="line">  <span class="token literal-property property">transformResponse</span><span class="token operator">:</span> <span class="token punctuation">[</span><span class="token keyword">function</span><span class="token punctuation">(</span><span class="token parameter">data</span><span class="token punctuation">)</span> <span class="token punctuation">{</span></span>
<span class="line">    <span class="token keyword">const</span> JSONbigToString <span class="token operator">=</span> <span class="token function">JSONbig</span><span class="token punctuation">(</span><span class="token punctuation">{</span><span class="token literal-property property">storeAsString</span><span class="token operator">:</span> <span class="token boolean">true</span><span class="token punctuation">}</span><span class="token punctuation">)</span></span>
<span class="line">    <span class="token keyword">return</span> <span class="token function">JSONbigToString</span><span class="token punctuation">(</span>data<span class="token punctuation">)</span></span>
<span class="line">  <span class="token punctuation">}</span><span class="token punctuation">]</span></span>
<span class="line"><span class="token punctuation">}</span><span class="token punctuation">)</span></span>
<span class="line"></span></code></pre>
</div><h4 id="变量类型判断" tabindex="-1"><a class="header-anchor" href="#变量类型判断"><span>变量类型判断</span></a></h4>
<div class="language-javascript" data-highlighter="prismjs" data-ext="js" data-title="js"><pre v-pre><code><span class="line"><span class="token keyword">typeof</span> <span class="token comment">//判断基础类型</span></span>
<span class="line"><span class="token keyword">instanceof</span> <span class="token comment">//判断对象是谁的实例constructor</span></span>
<span class="line"><span class="token class-name">Object</span><span class="token punctuation">.</span>prototype<span class="token punctuation">.</span>toString<span class="token punctuation">.</span>call <span class="token comment">//判断具体类型返回字符串</span></span>
...
<li>在Java和Python中，多线程可以解决异步编程问题，通过创建多个线程来执行不同的任务，从而避免阻塞主线程。</li>
<li>Node.js则通过异步编程方法来实现非阻塞的I/O操作，使得高并发的程序能够更好地利用系统资源。</li>
<li>异步编程中，回调函数是解决异步操作返回结果的关键，回调函数可以指定在异步操作完成后执行的代码，从而使得异步操作能够更好地与主程序结合起来。</li>
<li>Promise是一种更先进的异步编程方法，它可以将异步操作包装成一个对象，并可以链式调用多个异步操作，使得代码更加简洁易读。</li>
</ol>
<h3 id="进程和线程" tabindex="-1"><a class="header-anchor" href="#进程和线程"><span>进程和线程</span></a></h3>
<p>js的执行调用栈是单线程，它在一个进程内只有一个主线程运行，通过事件循环和回调函数实现非阻塞I/O和并发操作，<em>单核执行(CPU单核最高) + 多个IO线程(硬盘读取)</em>，异步：IO读取的时候cpu空闲，执行其它脚本，事件驱动和多线程的方式提高IO的效率</p>
<p>进程(厂房)程序运行的环境：进程(process)是程序运行的环境,指正在运行的一个程序，它包含了程序执行的代码和所需的资源，是程序运行的基本单位</p>
<p>线程(工人)线程是实际进行运算的代码: 线程(Thread)是进程中的一个执行流，用于执行任务JavaScript的主线程负责执行代码，处理事件等</p>
<h4 id="i-o操作" tabindex="-1"><a class="header-anchor" href="#i-o操作"><span>I/O操作</span></a></h4>
<p>文件系统 I/O: 使用fs模块进行同步或异步的文件读写操作
网络 I/O: HTTP、HTTPS、TCP 和 UDP
数据库 I/O: MySQL、PostgreSQL等数据库的操作和管理</p>
<div class="hint-container tip">
<p class="hint-container-title">提示</p>
<ul>
<li>时间复杂度：O(n) n=函数执行次数</li>
<li>空间复杂度：O(n) n=栈中存有函数</li>
</ul>
</div>
<h3 id="global属性" tabindex="-1"><a class="header-anchor" href="#global属性"><span>Global属性</span></a></h3>
<p>默认声明的参数不放在global上面，每个人文件都是一个模板，模块外面包着一个匿名函数(funciton(){})()</p>
<p>全局属性：global中的属性</p>
<ul>
<li>process: Node进程信息，可以获取进程环境信息，内存使用情况</li>
<li>Buffer: 处理二进制数据，Buffer对象是固定长度的字节数组，用来表示原始数据</li>
<li>platform: 返回操作系统的标识，window-&gt;win32  mac-&gt;'darwin'</li>
<li>argv: 含命令行参数，默认两个参数，第一个是可执行文件路径，第二个是脚本文件的路径</li>
<li>cwd: 当前工作目录，current working directory</li>
<li>env: 环境变量，windows通过set设置，macos通过export的设置</li>
<li>setInterval/setTimeout: 宏任务的定时器</li>
<li>nextTicknode: 微任务</li>
</ul>
<p>全局对象：</p>
<ul>
<li>module: 每个文件都视为一个模块，module对象包含了当前模块相关的有用信息和方法</li>
<li>exports: 是module.exports的引用，用于导出模块的功能，可通过exports向其他模块暴露接口</li>
<li>require: 用于加载模块，解释一个模块的标识符为参数，返回被请求模块的导出对象</li>
<li>__dirname: 当前模块的目录名</li>
<li>__filename: 当前模块的文件名(完整路径)</li>
</ul>
<h2 id="typescript" tabindex="-1"><a class="header-anchor" href="#typescript"><span>TypeScript</span></a></h2>
<p>TS可以视为一组具有共同点的值，因为类型只是集合，所以某个特定值可以同时属于多个集合，例如Number类型就是集合里包含所有的数值，Boolean集合只有两个元素，undefined和null分别只有一个元素，Object类型集合包含所有类对象数据结构，函数，日期，正则，对象等</p>
<h3 id="集合之间的相等、交集、补集" tabindex="-1"><a class="header-anchor" href="#集合之间的相等、交集、补集"><span>集合之间的相等、交集、补集</span></a></h3>
<ul>
<li>交集 C∈A C∈B C是A和B集合的交集 C = A∩B</li>
<li>并集 A和B集合的所有元素合并 C = A∪B</li>
<li>补集 C集合不属于A集合的元素 C = A'</li>
</ul>
<h3 id="集合运算" tabindex="-1"><a class="header-anchor" href="#集合运算"><span>集合运算</span></a></h3>
<ul>
<li>幂等律: A∪A=A,A∩A=A</li>
<li>交换律: A∩B=B∩A,A∪B=B∪A</li>
<li>结合律: (A∪B)∪C=A∪(B∪C),(A∩B)∩C=A∩(B∩C)</li>
<li>分配律: A∪(B∩C)=(A∪B)∩(A∪C),A∩(B∪C)=(A∩B)∪(A∩C)</li>
</ul>
<h3 id="typescript9种运算方式" tabindex="-1"><a class="header-anchor" href="#typescript9种运算方式"><span>Typescript9种运算方式</span></a></h3>
<ul>
<li>条件: T extends U ? X : Y</li>
<li>约束: extends</li>
<li>推导: infer</li>
<li>联合: |   // T | B 表示包含T或者C即可，也可以TB都有</li>
<li>交叉: &amp;  // T | B 表示必须包含TB所有合并类型</li>
<li>索引查询: Keyof T</li>
<li>索引访问: T[K]</li>
<li>索引遍历: in</li>
<li>索引重映射: as</li>
</ul>
<p><strong>🤖子类型比父类型更加具体，父类型比子类型更宽泛</strong></p>
<h3 id="访问修饰符" tabindex="-1"><a class="header-anchor" href="#访问修饰符"><span>访问修饰符</span></a></h3>
<ul>
<li>+public: 公共修饰符，其属性和方法都是公有的</li>
<li>-rivate: 私有修饰符，其修饰的属性和方法在class外不可见</li>
<li>#protected:  受保护修饰符，其修饰的属性和方法在当前类和子类内部可以访问</li>
<li>readonly: 只读修饰符</li>
<li>static: 静态修饰符，类直接调用</li>
</ul>
<div class="language-typescript" data-highlighter="prismjs" data-ext="ts" data-title="ts"><pre v-pre><code><span class="line"><span class="token comment">// 类型断言</span></span>
<span class="line">source <span class="token keyword">as</span> newType</span>
<span class="line"><span class="token operator">&lt;</span>newType<span class="token operator">></span>source</span>
<span class="line"></span>
<span class="line"><span class="token comment">// 加减修饰符</span></span>
<span class="line"><span class="token keyword">type</span> <span class="token class-name"><span class="token constant">T0</span><span class="token operator">&lt;</span><span class="token constant">T</span><span class="token operator">></span></span> <span class="token operator">=</span> <span class="token punctuation">{</span><span class="token operator">-</span><span class="token keyword">readonly</span> <span class="token punctuation">[</span><span class="token constant">P</span> <span class="token keyword">in</span> <span class="token keyword">keyof</span> <span class="token constant">T</span><span class="token punctuation">]</span><span class="token operator">-</span><span class="token operator">?</span><span class="token operator">:</span> <span class="token constant">T</span><span class="token punctuation">[</span><span class="token constant">P</span><span class="token punctuation">]</span><span class="token punctuation">}</span></span>
<span class="line"><span class="token keyword">type</span> <span class="token class-name"><span class="token constant">T1</span><span class="token operator">&lt;</span><span class="token constant">T</span><span class="token operator">></span></span> <span class="token operator">=</span> <span class="token punctuation">{</span><span class="token operator">-</span><span class="token keyword">readonly</span> <span class="token punctuation">[</span><span class="token constant">P</span> <span class="token keyword">in</span> <span class="token keyword">keyof</span> <span class="token constant">T</span><span class="token punctuation">]</span><span class="token operator">+</span><span class="token operator">?</span><span class="token operator">:</span> <span class="token constant">T</span><span class="token punctuation">[</span><span class="token constant">P</span><span class="token punctuation">]</span><span class="token punctuation">}</span></span>
<span class="line"></span></code></pre>
</div><h3 id="协变逆变" tabindex="-1"><a class="header-anchor" href="#协变逆变"><span>协变逆变</span></a></h3>
<ul>
<li>co-variance: 协变指的是在某种结构中，如果 B 是 A 的子类型，那么含有 B 类型参数的泛型也是含有 A 类型参数的泛型的子类型。简单来说，就是子类型可以替换父类型。</li>
<li>contra-variance: 逆变则相反，它逆转了子类型关系。也就是说，如果 B 是 A 的子类型，那么含有 B 类型参数的泛型是含有 A 类型参数的泛型的父类型。</li>
</ul>
<h3 id="运算套路" tabindex="-1"><a class="header-anchor" href="#运算套路"><span>运算套路</span></a></h3>
<div class="language-typescript" data-highlighter="prismjs" data-ext="ts" data-title="ts"><pre v-pre><code><span class="line"><span class="token comment">// 模式匹配做提取</span></span>
<span class="line"><span class="token keyword">type</span> <span class="token class-name">GetParameters<span class="token operator">&lt;</span>Func <span class="token keyword">extends</span> <span class="token builtin">Function</span><span class="token operator">></span></span> <span class="token operator">=</span> Func <span class="token keyword">extends</span> <span class="token punctuation">(</span><span class="token operator">...</span>args<span class="token operator">:</span><span class="token keyword">infer</span> Args<span class="token punctuation">)</span> <span class="token operator">=></span> <span class="token builtin">unknown</span> <span class="token operator">?</span> args <span class="token operator">:</span> <span class="token builtin">never</span></span>
<span class="line"><span class="token keyword">type</span> <span class="token class-name">ParamenterResult</span> <span class="token operator">=</span> GetParameters<span class="token operator">&lt;</span><span class="token punctuation">(</span>name<span class="token operator">:</span><span class="token builtin">string</span><span class="token punctuation">)</span> <span class="token operator">=></span> <span class="token builtin">string</span><span class="token operator">></span></span>
<span class="line"><span class="token comment">// 首先通过extends限制类型必须是Function</span></span>
<span class="line"><span class="token comment">// 然后用extends为参数类型匹配公式，当满足公式时提取公式中的变量Args</span></span>
<span class="line"></span>
<span class="line"><span class="token comment">// 重新解构做变换</span></span>
<span class="line"><span class="token keyword">type</span> <span class="token class-name">CapitalizeStr<span class="token operator">&lt;</span>Str <span class="token keyword">extends</span> <span class="token builtin">string</span><span class="token operator">></span></span> <span class="token operator">=</span> Str <span class="token keyword">extends</span> <span class="token template-string"><span class="token template-punctuation string">`</span><span class="token interpolation"><span class="token interpolation-punctuation punctuation">${</span><span class="token keyword">infer</span> First<span class="token interpolation-punctuation punctuation">}</span></span><span class="token interpolation"><span class="token interpolation-punctuation punctuation">${</span><span class="token keyword">infer</span> Rest<span class="token interpolation-punctuation punctuation">}</span></span><span class="token template-punctuation string">`</span></span><span class="token operator">?</span><span class="token template-string"><span class="token template-punctuation string">`</span><span class="token interpolation"><span class="token interpolation-punctuation punctuation">${</span>Uppercase<span class="token operator">&lt;</span>First<span class="token operator">></span><span class="token interpolation-punctuation punctuation">}</span></span><span class="token interpolation"><span class="token interpolation-punctuation punctuation">${</span>Test<span class="token interpolation-punctuation punctuation">}</span></span><span class="token template-punctuation string">`</span></span> <span class="token operator">:</span> Str</span>
<span class="line"><span class="token keyword">type</span> <span class="token class-name">Capitalize</span> <span class="token operator">=</span> CapitalizeStr<span class="token operator">&lt;</span><span class="token string">'tang'</span><span class="token operator">></span></span>
<span class="line"><span class="token comment">// 限制输入的类型必须时字符串类型</span></span>
<span class="line"><span class="token comment">// 用extends为参数类型匹配公式，提取公式中的变量First Rest,并通过Uppercase封装</span></span>
<span class="line"></span>
<span class="line"><span class="token comment">// 递归复用做循环</span></span>
<span class="line"><span class="token keyword">type</span> <span class="token class-name">ReverseArr<span class="token operator">&lt;</span>Arr <span class="token keyword">extends</span> unKnown<span class="token punctuation">[</span><span class="token punctuation">]</span><span class="token operator">></span></span> <span class="token operator">=</span> Arr <span class="token keyword">extends</span> <span class="token punctuation">[</span><span class="token keyword">infer</span> First<span class="token punctuation">,</span> <span class="token operator">...</span><span class="token keyword">infer</span> Rest<span class="token punctuation">]</span> <span class="token operator">?</span> <span class="token punctuation">[</span><span class="token operator">...</span>ReverseArr<span class="token operator">&lt;</span>Rest<span class="token operator">></span><span class="token punctuation">,</span>First<span class="token punctuation">]</span><span class="token operator">:</span>Arr</span>
<span class="line"><span class="token keyword">type</span> <span class="token class-name">ReverseArrResult</span> <span class="token operator">=</span>ReverseArr<span class="token operator">&lt;</span><span class="token punctuation">[</span><span class="token number">1</span><span class="token punctuation">,</span><span class="token number">2</span><span class="token punctuation">,</span><span class="token number">3</span><span class="token punctuation">,</span><span class="token number">4</span><span class="token punctuation">]</span><span class="token operator">></span></span>
<span class="line"><span class="token comment">// 限制必须是数组，用extends匹配公式，满足条件，则调用自身，否则直接返回</span></span>
<span class="line"></span>
<span class="line"><span class="token comment">// 数组长度做计数</span></span>
<span class="line"><span class="token keyword">type</span> <span class="token class-name">BuildArray<span class="token operator">&lt;</span>Length <span class="token keyword">extends</span> <span class="token builtin">number</span><span class="token punctuation">,</span> Ele <span class="token operator">=</span> <span class="token builtin">unknown</span><span class="token punctuation">,</span> Arr <span class="token keyword">extends</span> <span class="token builtin">unknown</span><span class="token punctuation">[</span><span class="token punctuation">]</span> <span class="token operator">=</span> <span class="token punctuation">[</span><span class="token punctuation">]</span><span class="token operator">></span></span> <span class="token operator">=</span> Arr<span class="token punctuation">[</span><span class="token string">'length'</span><span class="token punctuation">]</span> <span class="token keyword">extends</span> <span class="token class-name">Length</span> <span class="token operator">?</span> Arr <span class="token operator">:</span> BuildArray<span class="token operator">&lt;</span>Length<span class="token punctuation">,</span> Ele<span class="token punctuation">,</span> <span class="token punctuation">[</span><span class="token operator">...</span>Arr<span class="token punctuation">,</span> Ele<span class="token punctuation">]</span><span class="token operator">></span></span>
<span class="line"><span class="token keyword">type</span> <span class="token class-name">Add<span class="token operator">&lt;</span>Num1 <span class="token keyword">extends</span> <span class="token builtin">number</span><span class="token punctuation">,</span> Num2 <span class="token keyword">extends</span> <span class="token builtin">number</span><span class="token operator">></span></span> <span class="token operator">=</span> <span class="token punctuation">[</span><span class="token operator">...</span>BuildArray<span class="token operator">&lt;</span>Num1<span class="token operator">></span><span class="token punctuation">,</span> <span class="token operator">...</span>BuildArray<span class="token operator">&lt;</span>Num2<span class="token operator">></span><span class="token punctuation">]</span><span class="token punctuation">[</span><span class="token string">'length'</span><span class="token punctuation">]</span></span>
<span class="line"><span class="token keyword">type</span> <span class="token class-name">AddResult</span> <span class="token operator">=</span> Add<span class="token operator">&lt;</span><span class="token number">32</span><span class="token punctuation">,</span><span class="token number">23</span><span class="token operator">></span></span>
